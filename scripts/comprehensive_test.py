...
def test_api_comprehensive():
    ...
    try:
        ...
        for gen_name, params in new_generators:
            payload = {"generator_type": gen_name, "count": 2, "parameters": params}
            response = requests.post(
                f"{base_url}/generate/{gen_name}", json=payload, timeout=5
            )
...
def test_data_validation():
    ...
    for gen_type, test_data, expected in validation_tests:
        try:
            ...
            print(
                f"   {status} {gen_type} 校验 '{test_data}': {result} (期望: {expected})"
            )
        except Exception as e:
