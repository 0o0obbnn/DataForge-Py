...
def test_marital_status_generator():
    ...
    # 测试1: 中国婚姻状况
    print("1. 中国婚姻状况测试:")
    generator_china = MaritalStatusGenerator({"region": "china", "include_age_factor": True})
    ...
    for age in test_ages:
        ...
        print(f"   年龄 {age} 岁权重: {[f'{w:.2f}' for w in weights]}")

    # 测试生成
    print("\n   生成示例:")
    for age in [20, 30, 40]:
        mock_context = type('MockContext', (), {'related_data': {'age': age}})()
        result = generator_china._generate_raw(mock_context)
        print(f"   年龄 {age} 岁: {result}")

    # 测试2: 国际婚姻状况
    print("\n2. 国际婚姻状况测试:")
    generator_us = MaritalStatusGenerator({"region": "us", "include_age_factor": False})
    ...
    results = []
    for _ in range(20):
        mock_context = type('MockContext', (), {'related_data': {'age': random.randint(20, 60)}})()
        result = generator_china._generate_raw(mock_context)
