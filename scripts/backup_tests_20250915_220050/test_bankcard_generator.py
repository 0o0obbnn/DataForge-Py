...
def test_basic_bankcard_generation():
    ...
    for i, card_number in enumerate(results, 1):
        ...
        print(f"  BIN检查: {generator._is_valid_bin(clean_number)}")
        print(f"  Luhn检查: {generator._validate_luhn(clean_number)}")
        ...
        digits = digits_of(clean_number)
        print(f"    数字列表: {digits}")
        odd_digits = digits[-1::-2]
        even_digits = digits[-2::-2]
        print(f"    奇数位(从右到左): {odd_digits}")
        print(f"    偶数位(从右到左): {even_digits}")
        checksum = sum(odd_digits)
        print(f"    奇数位和: {checksum}")
        for d in even_digits:
            doubled = d * 2
            digit_sum = sum(digits_of(doubled))
            checksum += digit_sum
            print(f"    偶数位{d} * 2 = {doubled}, 数字和: {digit_sum}")
        print(f"    总校验和: {checksum}")
        print(f"    校验和模10: {checksum % 10}")

        # 检查生成过程中的校验位计算
        print("  生成过程调试:")
        # 获取BIN前缀
        bin_prefix = generator._get_bin_prefix()
        print(f"    BIN前缀: {bin_prefix}")
        ...
        print(f"    剩余位数: {remaining_length}")

        # 生成随机数字
        # import random
        # random.seed(42)  # 固定种子以便调试
        random_digits = "".join(
            [str(random.randint(0, 9)) for _ in range(remaining_length - 1)]
        )
        print(f"    随机数字: {random_digits}")

        # 生成基础卡号（不含校验位）
        base_number = bin_prefix + random_digits
        print(f"    基础卡号: {base_number}")

        # 计算校验位
        check_digit = generator._calculate_luhn_check_digit(base_number)
        print(f"    校验位: {check_digit}")

        # 完整卡号
        full_card_number = base_number + str(check_digit)
        print(f"    完整卡号: {full_card_number}")

        # 验证完整卡号
        is_valid_full = generator._validate_luhn(full_card_number)
        print(f"    完整卡号Luhn校验: {'✅' if is_valid_full else '❌'}")
