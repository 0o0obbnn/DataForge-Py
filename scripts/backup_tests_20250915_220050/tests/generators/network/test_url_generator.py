...
from dataforge.core.generator import GeneratorType
...
class TestURLGenerator:
    ...
    def test_keyword_query_generation(self):
        """测试关键词查询参数生成"""
        generator = URLGenerator(query_style="KEYWORD", query_params_count=(2, 2))
        url = generator.generate()
...
class TestGenericURLGenerator:
    ...
    def test_registration(self):
        ...
        assert generator.generator_type == GeneratorType.NETWORK
        assert "url" in str(generator.supported_parameters)
    ...
    def test_parameter_support(self):
        ...
        assert "include_path" in params
        assert "include_query" in params
