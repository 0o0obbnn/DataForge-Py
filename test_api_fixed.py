...
def test_api_endpoints():
    ...
    try:
        ...
        # 检查新增的生成器
        new_generators = ['license_plate', 'company_name', 'uscc', 'organization_code', 'lei']
        found_generators = []
        ...
        for case in test_cases:
            try:
                ...
                response = requests.post(
                    f"{base_url}/generate/{case['generator']}",
                    json=payload,
                    timeout=5
                )
        ...
        batch_payload = {
            "generators": [
                {"type": "license_plate", "count": 2, "parameters": {"type": "NEW_ENERGY"}},
                {"type": "company_name", "count": 2, "parameters": {"industry": "FINANCE"}},
        ...
        for generator_name in ['license_plate', 'company_name']:
            try:
                response = requests.get(f"{base_url}/generators/{generator_name}", timeout=5)
                if response.status_code == 200:
                    info = response.json()
