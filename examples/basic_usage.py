...
from dataforge import default_factory, GeneratorConfig, GenerationContext
...
def example_invalid_data():
    ...
    for i, id_card in enumerate(invalid_ids, 1):
        print(f"  {i}. {id_card}")
        print(f"     校验结果: {'✓' if generator.validate(id_card) else '✗ (预期无效)'}")
...
def example_output_formatting():
    ...
    # 手机号
    phone_config = GeneratorConfig(generator_type='phone', parameters={'operator': 'UNICOM'})
...
def example_related_data():
    ...
    id_config = GeneratorConfig(
        generator_type='idcard',
        parameters={
            'birth_date_range': ('1990-01-01', '1995-12-31')
        }
    )
