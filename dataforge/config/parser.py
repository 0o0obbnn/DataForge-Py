...
from ..core.factory import GeneratorRegistry, default_registry
...
class ConfigSchema(BaseModel):
    ...
    generator_type: str
    count: int = 1
    should_validate: bool = True  # 重命名以避免阴影BaseModel.validate方法
    unique: bool = False
...
class ConfigParser:
    ...
    def __init__(self, registry: GeneratorRegistry = default_registry):
    ...
    def parse_config(self, config_data: dict[str, Any]) -> list[GeneratorConfig]:
        ...
        try:
            ...
            if "generator_type" in config_data:
                ...
                config = self._parse_single_config(schema.model_dump())
                configs.append(config)
            # 支持多个生成器配置
            elif "generators" in config_data:
                multi_schema = MultiConfigSchema(**config_data)
                for gen_config in multi_schema.generators:
                    config = self._parse_single_config(gen_config.model_dump())
                    configs.append(config)
            else:
                raise ValueError(
                    "配置文件格式错误：必须包含 'generator_type' 或 'generators' 字段"
                )
    ...
    def _parse_single_config(self, config_data: dict[str, Any]) -> GeneratorConfig:
        ...
        # 验证生成器类型是否已注册
        if not self.registry.is_registered(generator_type):
            available_generators = self.registry.list_generators()
            raise ValueError(
                f"未知的生成器类型: {generator_type}. 可用的生成器: {', '.join(available_generators)}"
            )
        ...
        return GeneratorConfig(
            ...
            validate=config_data.get("should_validate", True),
    ...
    def create_template_config(self, generator_types: list[str]) -> dict[str, Any]:
        ...
        if len(generator_types) == 1:
            ...
            return {
                ...
                "output": {"format": "json", "file": "output.json", "pretty": True},
            }
    ...
    def _get_default_parameters(self, generator_type: str) -> dict[str, Any]:
        ...
        defaults = {
            ...
            "phone": {"region": "CN", "operator": "ANY", "prefix": None, "valid": True},
            "name": {
                "type": "BOTH",
                "gender": "ANY",
                "surname_file": None,
    ...
    def _validate_generator_parameters(
        self, generator_type: str, parameters: dict[str, Any]
    ) -> None:
        """验证生成器参数"""
        # 创建临时配置来检查参数有效性
        ...
        if generator_class:
            ...
            if unknown_params:
                logger.warning(
                    f"Unknown parameters for {generator_type}: {', '.join(unknown_params)}"
                )
    ...
    def load_config_file(self, file_path: str | Path) -> dict[str, Any]:
        """加载配置文件"""
        path = Path(file_path)
