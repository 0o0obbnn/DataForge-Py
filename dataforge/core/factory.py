...
class GeneratorRegistry:
    ...
    def register(
        ...
        if not issubclass(generator_class, DataGenerator):
            raise GeneratorConfigError(f"Generator class must inherit from DataGenerator: {generator_class}")
...
class GeneratorFactory:
    ...
    def __init__(self, registry: GeneratorRegistry, relation_manager: Optional[DataRelationManager] = None) -> None:
    ...
    def generate_batch_with_relations(
        ...
        try:
            ...
            ordered_fields = self.relation_manager.get_relation_dependencies(field_names)
            ...
            raise GeneratorConfigError(f"Batch generation with relations failed: {str(e)}")
...
def register_generator(name: str, aliases: Optional[list[str]] = None):
    ...
    def decorator(generator_class: Type[DataGenerator[Any]]) -> Type[DataGenerator[Any]]:
