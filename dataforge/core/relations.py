...
class DataRelationManager:
    ...
    def _setup_builtin_rules(self):
        ...
        # 性别 -> 姓名关联
        def gender_to_name_constraint(gender: str, context: GenerationContext) -> Dict[str, Any]:
            """性别约束姓名生成"""
            return {'gender': gender}
        
        # 年龄 -> 身份证关联
        def age_to_idcard_constraint(age: int, context: GenerationContext) -> Dict[str, Any]:
            """年龄约束身份证生成"""
            current_year = datetime.now().year
        ...
        # 地址 -> 身份证地区关联  
        def address_to_idcard_region(address: str, context: GenerationContext) -> Dict[str, Any]:
            """地址约束身份证地区"""
            # 简化实现：从地址中提取省份信息
            province_mapping = {
                '北京': '110000',
    ...
    def add_builtin_relation(self, source_field: str, target_field: str, 
                           relation_type: str = 'derive', priority: int = 0):
        """添加内置关联规则"""
    ...
    def apply_relations(self, generated_data: Dict[str, Any], 
        ...
        for config in pending_configs:
            ...
            # 查找该字段的关联规则
            applicable_rules = [
                rule for rule in self.relation_rules 
                if rule.target_field == target_field and rule.source_field in result_data
            ]
            ...
            if applicable_rules:
                ...
                try:
                    ...
                    # 关联规则执行失败时记录错误但不中断生成
                    print(f"Warning: Relation rule failed for {rule.source_field} -> {rule.target_field}: {e}")
...
default_relation_manager.add_builtin_relation('idcard', 'birthdate', 'derive', priority=10)
...
default_relation_manager.add_builtin_relation('address', 'idcard', 'constrain', priority=3)