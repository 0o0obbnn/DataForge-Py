...
class OptimizedNameGenerator(ValidatedDataGenerator[str]):
    """优化的中文姓名生成器"""
    ...
    def common_surnames(self):
        """获取常见姓氏"""
        return self.surnames_data.get('common_surnames', [])
    
    @property
    def rare_surnames(self):
        """获取罕见姓氏"""
        return self.surnames_data.get('rare_surnames', [])
    
    @property
    def male_names(self):
        """获取男性名字"""
        return self.givennames_data.get('male_names', [])
    
    @property
    def female_names(self):
        """获取女性名字"""
        return self.givennames_data.get('female_names', [])
    
    @property
    def neutral_names(self):
        """获取中性名字"""
        return self.givennames_data.get('neutral_names', [])
    ...
    def _select_surname(self) -> Dict[str, Any]:
        """选择姓氏"""
        # 是否选择复姓
        if random.random() < self.compound_surname_ratio and self.rare_surnames:
            return random.choice(self.rare_surnames)
    ...
    def _select_given_name(self, gender: str) -> str:
        ...
        if gender == 'MALE':
            ...
            name_pool = random.choice([pool for pool in all_pools if pool])
        
        if not name_pool:
            return '明'  # 默认名字
        
        name_info = random.choice(name_pool)
        return name_info.get('name', name_info) if isinstance(name_info, dict) else str(name_info)
    ...
    def supported_parameters(self) -> List[str]:
        return [
            'type', 'gender', 'surname_file', 'givenname_file',
            'include_pinyin', 'compound_surname_ratio'
        ]
