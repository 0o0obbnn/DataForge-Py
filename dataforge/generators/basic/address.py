...
class AddressGenerator(DataGenerator[str]):
    ...
    def _setup(self) -> None:
        ...
        self.detail_level = self.parameters.get(
            "detail_level", "FULL"
        )  # STREET, COMMUNITY, FULL
    ...
    def _find_province_by_city(self, city_name: str) -> dict[str, str]:
        ...
        for province in self.regions_data["provinces"]:
            ...
            cities_data = self.regions_data.get("major_cities", {}).get(
                province_code, []
            )
            ...
            for city in cities_data:
                if (
                    city["name"] == city_name
                    or city_name in city["name"]
                    or city["name"].startswith(city_name)
                ):
                    return province
    ...
    def _select_province(self) -> dict[str, str]:
        ...
        if self.province:
            ...
            for province in self.regions_data["provinces"]:
                if (
                    province["name"] == self.province
                    or province["code"] == self.province
                    or province["short"] == self.province
                    or self.province in province["name"]
                    or province["name"].startswith(self.province)
                ):
                    return province
    ...
    def _select_city_district(self, province_info: dict[str, str]) -> tuple:
        ...
        if self.city:
            for city in cities_data:
                if (
                    city["name"] == self.city
                    or self.city in city["name"]
                    or city["name"].startswith(self.city)
                ):
                    city_data = city
                    break
        ...
        city_info = {"name": city_data["name"], "code": city_data["code"]}
    ...
    def _generate_detailed_address(self) -> dict[str, str]:
        ...
        if self.detail_level in ["STREET", "COMMUNITY", "FULL"]:
            ...
            direction = (
                secrets.choice(self.directions)
                if (secrets.randbelow(1000000) / 1000000) < 0.3
                else ""
            )
            street_type = secrets.choice(
                self.regions_data.get("street_types", ["路", "街"])
            )
        ...
        if self.detail_level in ["COMMUNITY", "FULL"]:
            # 生成小区/社区
            prefix = (
                secrets.choice(self.address_prefixes)
                if (secrets.randbelow(1000000) / 1000000) < 0.4
                else ""
            )
        ...
        if self.detail_level == "FULL":
            ...
            building_unit = (
                secrets.choice(self.building_units)
