...
class CommunicationGenerator(DataGenerator[Union[str, Dict[str, Any]]]):
    ...
    def _setup(self) -> None:
        """初始化通讯方式生成器参数"""
        self.comm_type = self.parameters.get("type", "mixed")  # phone, email, social_media, mixed
    ...
    def _generate_email(self) -> Union[str, Dict[str, str]]:
        ...
        username = "".join(secrets.choice("abcdefghijklmnopqrstuvwxyz0123456789") 
    ...
    def _generate_social_media(self) -> Union[str, Dict[str, str]]:
        ...
        if platform in ["WeChat", "QQ"]:
            ...
            if secrets.randbelow(2) == 0:
                account = "".join(str(secrets.randbelow(10)) for _ in range(secrets.randbelow(5) + 6))
            else:
                account = "".join(secrets.choice("abcdefghijklmnopqrstuvwxyz0123456789_") 
        ...
        if self.include_label:
            return {"type": "social_media", "value": account, "platform": platform, "label": platform}
