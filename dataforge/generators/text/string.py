...
class StringGenerator(DataGenerator[str]):
    ...
    def _setup(self) -> None:
        ...
        self.must_include = self.parameters.get(
            "must_include", []
        )  # 必须包含的字符类型
        self.case_style = self.parameters.get(
            "case", "MIXED"
        )  # UPPER, LOWER, MIXED, TITLE
    ...
    def _generate_raw(self, context: Optional[GenerationContext] = None) -> str:
        ...
        if self.allow_repeats:
            ...
            if length > len(chars):
                raise ValueError(
                    f"无法生成长度为{length}的不重复字符串，可用字符只有{len(chars)}个"
                )
    ...
    def _get_character_set(self) -> list[str]:
        ...
        if self.custom_charset:
            ...
            chars = list(
                self.CHAR_SETS.get(self.charset.upper(), self.CHAR_SETS["ALPHANUMERIC"])
            )
    ...
    def _generate_with_pattern(self) -> str:
        ...
        for char in pattern:
            if char == "#":  # 数字
                ...
                result += secrets.choice(string.ascii_letters)
            elif char == "*":  # 任意字符
...
class BooleanGenerator(DataGenerator[Union[bool, str]]):
    ...
    def _setup(self) -> None:
        ...
        self.output_format = self.parameters.get(
            "format", "BOOLEAN"
        )  # BOOLEAN, STRING, NUMERIC, CHINESE
        self.custom_values = self.parameters.get(
            "custom_values", None
        )  # 自定义True/False值
    ...
    def _generate_raw(
        ...
        # 生成布尔值
        bool_value = (secrets.randbelow(1000000) / 1000000) < self.true_ratio
