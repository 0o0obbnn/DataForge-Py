...
class LEIGenerator(ValidatedDataGenerator[str]):
    """LEI码生成器 (ISO 17442标准)"""
    ...
    def _generate_raw(self, context: Optional[GenerationContext] = None) -> str:
        """生成原始LEI码"""
        # 1. LOU ID (4位)
        lou_id = random.choice(self.lou_ids)
    ...
    def generate_with_info(self, context: Optional[GenerationContext] = None) -> dict:
        """生成LEI码并返回详细信息"""
        lei = self._generate_raw(context)
...
@register_generator('lei', ['legal_entity_identifier', 'LEI码', '法人机构识别码'])
