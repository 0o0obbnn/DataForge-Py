        ...
        try:
            if self.format == 'iana':
                ...
                if data in self.timezones or data.endswith(' (DST)'):
                    return True
                # 检查是否包含有效区域前缀
                if any(data.startswith(prefix) for prefix in ['Asia/', 'America/', 'Europe/', 'Pacific/', 'Africa/', 'Australia/']):
                    return True
                ...
                if data.startswith(('UTC+', 'UTC-', 'GMT+', 'GMT-')):
                    try:
                        ...
                        if offset_part and (offset_part.isdigit() or (offset_part[0] in '+-' and offset_part[1:].isdigit())):
                            return True
                ...
                # 时区缩写验证
                common_abbrevs = ['UTC', 'GMT', 'CET', 'CEST', 'EET', 'EEST', 'WET', 'WEST',
                                 'EST', 'EDT', 'CST', 'CDT', 'MST', 'MDT', 'PST', 'PDT',
                                 'JST', 'KST', 'SGT', 'HKT', 'IST', 'AEST', 'AEDT',
                                 'NZST', 'NZDT']
...
class TimezoneGenerator(DataGenerator[str]):
    ...
    def __init__(self, config: GeneratorConfig):
        ...
        # 根据区域筛选时区
        self.regional_timezones = self._get_regional_timezones()

    def _setup(self) -> None:
        """配置生成器参数"""
        self.format = self.config.parameters.get('format', 'iana')
        self.region = self.config.parameters.get('region', 'global')
        self.include_dst = self.config.parameters.get('include_dst', False)
    ...
    def _generate_iana(self) -> str:
        """生成IANA时区标识"""
        timezone = secrets.choice(self.regional_timezones)
    ...
    def _generate_abbreviation(self) -> str:
        ...
        abbreviations = [
            ...
            'EST', 'EDT', 'CST', 'CDT', 'MST', 'MDT', 'PST', 'PDT',
            'JST', 'KST', 'CST', 'SGT', 'HKT', 'IST', 'AEST', 'AEDT',
            'NZST', 'NZDT'
        ]
