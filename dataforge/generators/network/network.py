...
class IPAddressGenerator(ValidatedDataGenerator[str]):
    """IP地址生成器"""
    
    def _setup(self) -> None:
        self.version = self.parameters.get('version', 4)  # 4 or 6
        self.ip_type = self.parameters.get('type', 'PUBLIC')  # PUBLIC, PRIVATE, LOOPBACK, MULTICAST
        self.subnet = self.parameters.get('subnet', None)  # 指定子网
        self.format_style = self.parameters.get('format', 'STANDARD')  # STANDARD, COMPRESSED (IPv6)
    ...
    def _generate_private_ipv4(self) -> str:
        """生成私有IPv4地址"""
        range_type = random.choice(self.ipv4_private_ranges)
    ...
    def _generate_private_ipv6(self) -> str:
        """生成私有IPv6地址"""
        prefix = random.choice(self.ipv6_private_prefixes)
        if prefix == 'fe80::':
            # Link-local地址
            ...
            # ULA地址
            suffix = ':'.join([f"{random.randint(0, 65535):x}" for _ in range(7)])
    ...
    def _generate_public_ipv6(self) -> str:
        """生成公网IPv6地址"""
        # 生成全球单播地址 (2000::/3)
        groups = [random.randint(0x2000, 0x3fff)]  # 第一组确保是全球单播
        groups.extend([random.randint(0, 65535) for _ in range(7)])
    ...
    def supported_parameters(self) -> List[str]:
        return ['version', 'type', 'subnet', 'format']


class MACAddressGenerator(ValidatedDataGenerator[str]):
    """MAC地址生成器"""
    
    def _setup(self) -> None:
        self.format_style = self.parameters.get('format', 'COLON')  # COLON, HYPHEN, NONE
    ...
    def _generate_raw(self, context: Optional[GenerationContext] = None) -> str:
        ...
        if self.vendor_oui:
            ...
            if isinstance(self.vendor_oui, str):
                ...
                oui_prefix = random.choice(vendor_ouis).replace(':', '')
        else:
            # 生成随机OUI
            oui_prefix = ''.join([f"{random.randint(0, 255):02x}" for _ in range(3)])
    ...
    @property
    def supported_parameters(self) -> List[str]:
        return ['format', 'case', 'vendor_oui', 'locally_administered']


class DomainGenerator(ValidatedDataGenerator[str]):
    """域名生成器"""
    
    def _setup(self) -> None:
        self.tld_type = self.parameters.get('tld_type', 'COMMON')  # COMMON, COUNTRY, ALL
        ...
        # 常用单词
        self.common_words = [
            'tech', 'data', 'cloud', 'web', 'app', 'soft', 'sys', 'info',
            'global', 'world', 'net', 'online', 'digital', 'smart', 'pro',
            'fast', 'secure', 'easy', 'simple', 'quick', 'best', 'top'
        ]
    ...
    def _generate_subdomains(self) -> List[str]:
        """生成子域名列表"""
        levels = random.randint(*self.subdomain_levels)
        subdomains = []
        
        for _ in range(levels):
            if self.use_real_words and random.random() < 0.6:
                subdomain = random.choice(['www', 'api', 'mail', 'ftp', 'blog', 'shop', 'news'])
            else:
    ...
    def _generate_main_domain(self) -> str:
        """生成主域名"""
        if self.use_real_words and random.random() < 0.7:
            # 使用真实单词
            ...
            # 生成随机字符串
            length = random.randint(*self.domain_length)
    ...
    def _select_tld(self) -> str:
        """选择顶级域名"""
        if self.custom_tlds:
            return random.choice(self.custom_tlds)
        
        if self.tld_type.upper() == 'COUNTRY':
            return random.choice(self.country_tlds)
        elif self.tld_type.upper() == 'ALL':
            all_tlds = self.common_tlds + self.country_tlds + self.new_tlds
            return random.choice(all_tlds)
        else:  # COMMON
            return random.choice(self.common_tlds)
    ...
    @property
    def supported_parameters(self) -> List[str]:
        return ['tld_type', 'subdomain_levels', 'domain_length', 'use_real_words', 'custom_tlds']


class PortNumberGenerator(ValidatedDataGenerator[int]):
    """端口号生成器"""
    
    def _setup(self) -> None:
        self.port_type = self.parameters.get('type', 'USER')  # SYSTEM, USER, DYNAMIC, COMMON
    ...
    def _generate_raw(self, context: Optional[GenerationContext] = None) -> int:
        ...
        # 获取端口范围
        port_range = self.port_ranges.get(self.port_type.upper(), self.port_ranges['USER'])
    ...
    def _generate_common_port(self) -> int:
        ...
        if self.protocol.upper() == 'TCP':
            return random.choice(self.common_ports['TCP'])
        elif self.protocol.upper() == 'UDP':
            return random.choice(self.common_ports['UDP'])
        else:  # ANY
            all_common = self.common_ports['TCP'] + self.common_ports['UDP'] + self.common_ports['BOTH']
            return random.choice(all_common)
    
    def _is_well_known_port(self, port: int) -> bool:
        """检查是否为知名端口"""
        all_common = (self.common_ports['TCP'] + 
    ...
    @property
    def supported_parameters(self) -> List[str]:
        return ['type', 'protocol', 'exclude_well_known']


@register_generator('ip_address', ['ip', 'IP地址'])
class GenericIPAddressGenerator(IPAddressGenerator):
    """通用IP地址生成器注册版本"""
    pass


@register_generator('mac_address', ['mac', 'MAC地址'])
class GenericMACAddressGenerator(MACAddressGenerator):
    """通用MAC地址生成器注册版本"""
    pass


@register_generator('domain', ['域名'])
class GenericDomainGenerator(DomainGenerator):
    """通用域名生成器注册版本"""
    pass


@register_generator('port', ['端口'])
