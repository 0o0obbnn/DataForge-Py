...
示例用法:
  ...
  dataforge generate idcard,age --count 10 --idcard.gender MALE --age.min 25 --age.max 45
        ...
        list_parser.add_argument('--type', choices=['basic', 'identifier', 'contact', 'network', 'all'], 
                               default='all', help='生成器类型')
    ...
    def _add_generate_args(self, parser: argparse.ArgumentParser):
        ...
        output_group.add_argument('--output-format', dest='output_format', choices=['csv', 'json', 'xml', 'sql', 'yaml'], 
                                default='json', help='输出格式')
        output_group.add_argument('--output-file', dest='output_file', help='输出文件路径')
        output_group.add_argument('--output-pretty', dest='output_pretty', action='store_true', help='美化输出')
        output_group.add_argument('--output-encoding', dest='output_encoding', default='utf-8', help='输出编码')
        
        # 数据校验参数
        validation_group = parser.add_argument_group('校验选项')
        validation_group.add_argument('--validate', action='store_true', default=True, help='启用数据校验')
        validation_group.add_argument('--no-validate', dest='validate', action='store_false', help='禁用数据校验')
        
        # 身份证参数
        idcard_group = parser.add_argument_group('身份证选项')
        idcard_group.add_argument('--idcard-region', dest='idcard_region', help='地区代码或名称')
        idcard_group.add_argument('--idcard-gender', dest='idcard_gender', choices=['MALE', 'FEMALE', 'ANY'], help='性别')
        idcard_group.add_argument('--idcard-birth-date-range', dest='idcard_birth_date_range', help='出生日期范围 (格式: YYYY-MM-DD,YYYY-MM-DD)')
        idcard_group.add_argument('--idcard-valid', dest='idcard_valid', type=bool, default=True, help='是否生成有效身份证')
        
        # 银行卡参数
        bankcard_group = parser.add_argument_group('银行卡选项')
        bankcard_group.add_argument('--bankcard-type', dest='bankcard_type', choices=['DEBIT', 'CREDIT', 'BOTH'], help='卡类型')
        bankcard_group.add_argument('--bankcard-issuer', dest='bankcard_issuer', help='发卡机构')
        bankcard_group.add_argument('--bankcard-bank', dest='bankcard_bank', help='银行代码')
        bankcard_group.add_argument('--bankcard-valid', dest='bankcard_valid', type=bool, default=True, help='是否符合Luhn算法')
        
        # 手机号参数
        phone_group = parser.add_argument_group('手机号选项')
        phone_group.add_argument('--phone-operator', dest='phone_operator', choices=['MOBILE', 'UNICOM', 'TELECOM', 'VIRTUAL', 'ANY'], help='运营商')
        phone_group.add_argument('--phone-prefix', dest='phone_prefix', help='号码前缀（逗号分隔）')
        phone_group.add_argument('--phone-valid', dest='phone_valid', type=bool, default=True, help='是否生成有效手机号')
        
        # 年龄参数
        age_group = parser.add_argument_group('年龄选项')
        age_group.add_argument('--age-min', dest='age_min', type=int, default=18, help='最小年龄')
        age_group.add_argument('--age-max', dest='age_max', type=int, default=60, help='最大年龄')
    ...
    def _handle_validate(self, args) -> int:
        ...
        try:
            ...
            print(f"配置文件 {args.config_file} 验证成功")
            return 0
        except Exception as e:
            print(f"配置文件验证失败: {e}")
            return 1
