...
def debug_api():
    ...
    try:
        ...
        response = requests.post(f"{base_url}/generate/license_plate", json=simple_payload, timeout=5)
        print(f"      状态: {response.status_code}")
        if response.status_code != 200:
            print(f"      错误: {response.text}")
        else:
        ...
        response = requests.post(f"{base_url}/generate/license_plate", json=param_payload, timeout=5)
        print(f"      状态: {response.status_code}")
        if response.status_code != 200:
            print(f"      错误: {response.text}")
        else:
