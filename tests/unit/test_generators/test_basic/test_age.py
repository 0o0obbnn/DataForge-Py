...
class TestAgeGenerator:
    ...
    def test_generate_single(self, generator_factory):
        """测试生成单个年龄"""
        config = GeneratorConfig(generator_type="age", parameters={})
    ...
    def test_with_min_max_parameters(self, generator_factory):
        ...
        config = GeneratorConfig(
            generator_type="age", parameters={"min": 20, "max": 30}
        )
        generator = generator_factory.create_generator(config)
        age = generator.generate_single()
    ...
    def test_child_age_range(self, generator_factory):
        """测试儿童年龄范围"""
        config = GeneratorConfig(generator_type="age", parameters={"min": 0, "max": 17})
        generator = generator_factory.create_generator(config)
        ages = generator.generate_batch(20)
    ...
    def test_elderly_age_range(self, generator_factory):
        """测试老年人年龄范围"""
        config = GeneratorConfig(
            generator_type="age", parameters={"min": 60, "max": 100}
        )
        generator = generator_factory.create_generator(config)
        ages = generator.generate_batch(20)
    ...
    def test_edge_cases(self, generator_factory):
        """测试边界情况"""
        # Test minimum age
        config = GeneratorConfig(generator_type="age", parameters={"min": 0, "max": 0})
        generator = generator_factory.create_generator(config)
        age = generator.generate_single()
        assert age == 0
        ...
        config = GeneratorConfig(
            generator_type="age", parameters={"min": 120, "max": 120}
        )
        generator = generator_factory.create_generator(config)
        age = generator.generate_single()
        assert age == 120
    ...
    def test_distribution(self, generator_factory):
        """测试年龄分布合理性"""
        config = GeneratorConfig(
            generator_type="age", parameters={"min": 0, "max": 100}
        )
        generator = generator_factory.create_generator(config)
