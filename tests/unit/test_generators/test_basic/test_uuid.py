...
class TestUUIDGenerator:
    ...
    def test_generate_single(self, generator_factory):
        """测试生成单个UUID"""
        config = GeneratorConfig(generator_type="uuid", parameters={})
    ...
    def test_uuid4_format(self, generator_factory):
        """测试UUID4格式"""
        config = GeneratorConfig(generator_type="uuid", parameters={"version": 4})
        generator = generator_factory.create_generator(config)
        result = generator.generate_single()
    ...
    def test_uppercase_format(self, generator_factory):
        """测试大写格式"""
        config = GeneratorConfig(generator_type="uuid", parameters={"uppercase": True})
        generator = generator_factory.create_generator(config)
        result = generator.generate_single()
    ...
    def test_no_hyphens(self, generator_factory):
        """测试无连字符格式"""
        config = GeneratorConfig(generator_type="uuid", parameters={"hyphens": False})
        generator = generator_factory.create_generator(config)
        result = generator.generate_single()
