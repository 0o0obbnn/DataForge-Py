...
class TestMaritalStatusGenerator:
    ...
    def test_international_options(self):
        """测试国际婚姻状况选项"""
        config = GeneratorConfig(generator_type="marital_status", parameters={"region": "us"})
        generator = MaritalStatusGenerator(config)
        options = generator._get_marital_status_options()
    ...
    def test_age_based_weights_young(self):
        """测试年轻人婚姻状况权重"""
        config = GeneratorConfig(generator_type="marital_status", parameters={"include_age_factor": True})
        generator = MaritalStatusGenerator(config)
    ...
    def test_generate_with_age_context(self):
        """测试带年龄上下文的生成"""
        config = GeneratorConfig(generator_type="marital_status", parameters={"region": "china", "include_age_factor": True})
        generator = MaritalStatusGenerator(config)
