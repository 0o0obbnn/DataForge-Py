...
class TestPasswordGenerator:
    ...
    def test_generate_single(self, generator_factory):
        ...
        config = GeneratorConfig(generator_type="password", parameters={})
    ...
    def test_password_length(self, generator_factory):
        ...
        config = GeneratorConfig(generator_type="password", parameters={"length": 16})
        generator = generator_factory.create_generator(config)
        password = generator.generate_single()
    ...
    def test_strong_password(self, generator_factory):
        ...
        config = GeneratorConfig(
            generator_type="password", parameters={"strength": "strong"}
        )
        generator = generator_factory.create_generator(config)
        password = generator.generate_single()
    ...
    def test_edge_cases(self, generator_factory):
        ...
        # Minimum length
        config = GeneratorConfig(generator_type="password", parameters={"length": 8})
        generator = generator_factory.create_generator(config)
        password = generator.generate_single()
        assert len(password) == 8

        # Maximum length
        config = GeneratorConfig(generator_type="password", parameters={"length": 32})
        generator = generator_factory.create_generator(config)
        password = generator.generate_single()
