...
class TestAddressGenerator:
    ...
    def test_generate_single(self, generator_factory):
        """测试生成单个地址"""
        config = GeneratorConfig(generator_type="address", parameters={})
    ...
    def test_province_filter(self, generator_factory):
        ...
        config = GeneratorConfig(
            generator_type="address", parameters={"province": "北京"}
        )
        generator = generator_factory.create_generator(config)
        address = generator.generate_single()
    ...
    def test_city_filter(self, generator_factory):
        """测试城市筛选"""
        config = GeneratorConfig(generator_type="address", parameters={"city": "上海"})
        generator = generator_factory.create_generator(config)
        address = generator.generate_single()
    ...
    def test_full_address(self, generator_factory):
        """测试完整地址"""
        config = GeneratorConfig(
            generator_type="address", parameters={"detail_level": "full"}
        )
        generator = generator_factory.create_generator(config)
        address = generator.generate_single()
    ...
    def test_simple_address(self, generator_factory):
        """测试简单地址"""
        config = GeneratorConfig(
            generator_type="address", parameters={"detail_level": "simple"}
        )
        generator = generator_factory.create_generator(config)
        address = generator.generate_single()
    ...
    def test_with_postal_code(self, generator_factory):
        """测试带邮编"""
        config = GeneratorConfig(
            generator_type="address", parameters={"include_postal_code": True}
        )
        generator = generator_factory.create_generator(config)
        address = generator.generate_single()
