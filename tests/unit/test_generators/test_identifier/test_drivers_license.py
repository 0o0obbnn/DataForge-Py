...
class TestDriversLicenseGenerator:
    ...
    def test_generate_single(self, generator_factory):
        """测试生成单个驾驶证号"""
        from dataforge.generators.identifier.drivers_license import DriversLicenseGenerator
    ...
    def test_license_format(self, generator_factory):
        ...
        assert len(license_num) == 18
        assert license_num.isdigit() or (license_num[:-1].isdigit() and license_num[-1] in 'X0123456789')
