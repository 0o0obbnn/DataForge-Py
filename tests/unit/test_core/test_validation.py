...
@pytest.mark.parametrize("id_card_number, expected_validity", [
...
def test_idcard_static_validation(generator_factory: GeneratorFactory, id_card_number: str, expected_validity: bool):
    """测试固定的身份证号校验"""
...
def test_idcard_invalid_generation(generator_factory: GeneratorFactory):
    ...
    assert any(not generator.validate(card) for card in invalid_cards), "生成的无效身份证中应至少有一个是无效的"

# --- 银行卡校验测试 ---

@pytest.mark.parametrize("card_number, expected_validity", [
...
def test_bankcard_static_validation(generator_factory: GeneratorFactory, card_number: str, expected_validity: bool):
    """测试固定的银行卡号Luhn校验"""
