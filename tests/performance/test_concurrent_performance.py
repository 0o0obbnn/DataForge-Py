...
class TestConcurrentPerformance:
    ...
    def test_concurrent_name_generation(self):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(generate_names, items_per_thread)
    ...
    def test_concurrent_multiple_generators(self):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(generate_profile, items_per_thread)
                for _ in range(num_threads)
            ]
            results = [f.result() for f in futures]
        end_time = time.time()
    ...
    def test_concurrent_scalability(self):
        ...
        for num_threads in thread_counts:
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                futures = [
                    executor.submit(generate_ages, items_per_thread)
                    for _ in range(num_threads)
                ]
                results = [f.result() for f in futures]
            end_time = time.time()
    ...
    def test_concurrent_id_generation(self):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(generate_ids, items_per_thread)
                for _ in range(num_threads)
            ]
            results = [f.result() for f in futures]
        end_time = time.time()
